...
|title: String
|Title of the breadcrumb item. If the menu has a `menuName` in the config, this will be used. Otherwise, the `displayName` from the content will be used.
...
|Returns breadcrumb items from the site root down to the content at the given path. Only content items marked as menu items ("Show in menu") are included. The `path` can be either a site-relative path (e.g. `/section/page`) or an absolute content path.
|===
...
To fetch breadcrumbs for a specific content path:

```graphql
query {
    guillotine {
        menu {
            breadcrumbs(path: "/section/page") {
                title
                path
                name
                content {
